...
const findMostUrgentSLAStatus = SLAStatuses => {
  ...
  SLAStatuses.sort((sla1, sla2) => Math.abs(sla1.threshold) - Math.abs(sla2.threshold));
...
const createSLAObject = (
  ...
  { first_reply_created_at: firstReplyCreatedAt, waiting_since: waitingSince, status } = {},
) => {
  ...
  return mostUrgent
    ? {
        ...
        threshold: formatSLATime(
          mostUrgent.threshold <= 0 ? -mostUrgent.threshold : mostUrgent.threshold,
        ),
