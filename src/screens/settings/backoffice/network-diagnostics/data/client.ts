...
import type {
  NetworkCaseStatus,
  NetworkCasesListParams,
  NetworkCasesListResponse,
  NetworkStatsResponse,
  NetworkCaseUpdateStatusResponse,
  NetworkCaseUpdateCommentResponse,
} from './types';
...
export function fetchNetworkCases(
  token: string,
  accountId: number,
  params: NetworkCasesListParams = {},
): Promise<NetworkCasesListResponse> {
  return postAction<NetworkCasesListResponse>(
    token,
    compact({ action: 'list', account_id: accountId, ...params }),
...
export function fetchNetworkStats(
  ...
  return postAction<NetworkStatsResponse>(
    token,
    compact({ action: 'stats', account_id: accountId, from, to }),
...
export function updateNetworkCaseStatus(
  ...
  return postAction<NetworkCaseUpdateStatusResponse>(
    token,
    { action: 'update_status', account_id: accountId, sk, status },
...
export function updateNetworkCaseComment(
  ...
  return postAction<NetworkCaseUpdateCommentResponse>(
    token,
    { action: 'update_comment', account_id: accountId, sk, comentario },
