...
import { type SearchSectionType, SEARCH_SECTIONS } from '@/screens/search/config';
...
export const selectSearchIsLoading = (state: RootState) => {
  ...
  return SEARCH_SECTIONS.some(section => state.search.sections[section.id]?.isLoading);
};
export const selectSearchIsCompleted = (state: RootState) => state.search?.isSearchCompleted || false;
